...
char Inventory::addCard(pc* pc, struct item* item, bool transaction, bool from_shop) {
	assert(pc);
	assert(item);
	...
	if (find_card != card_.end()) {
		...
		if (transaction) {
			add_transaction(item->item_type, (*find_card), old_amount);
		}

		if (from_shop) {
			show_buyitem(pc, (*find_card));
		}
		return ADDITEM_SUCCESS;
	}
	...
	// Push item to vector
	std::shared_ptr<INV_CARD> new_item(new INV_CARD());
	new_item->id = last_id;
